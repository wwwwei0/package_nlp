  ...
  init: function (encode = 1) {
    try {
      if(isSENTIMENTInited){
        ...
        if(!status){
          throw new Error('data path is error, or license is error.')
        }
        isSENTIMENTInited = true;
        console.log('sentiment init success.');
        return !!status;
      }
    } catch (error) {
      console.error('sentiment init error.');
      console.error('please update license.');
      console.log(error);
      return false;
    }
  },
