...
module.exports = {
  ...
  /**
   * sentiment object in paragraph.
   * @method getOneObjectResult
   * @param {string} obj - the object to sentiment.
   * @param {string} content - the paragraph content to sentiment.
   * @param {string} title - the title of paragraph to sentiment.
   * @return {string} xml - the result xml string.
   */
  getOneObjectResult: function (obj, content, title = '') {
    try {
      if(!isInited){
        console.log('no init...');
        return;
      }else{
        let xml = sentiment.ST_GetOneObjectResult(title, content, obj);
        return xml;
      }
    } catch (error) {
